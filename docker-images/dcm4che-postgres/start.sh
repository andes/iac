...
function initialize_primary() {
    ...
    if [ ! -f ${PGDATA?}/postgresql.conf ]; then
        ...
        # DCM4CHE
        echo_info "Loading DCM4CHE data"
        psql "${PG_DATABASE}" -U postgres -p "${PG_PRIMARY_PORT}" -a -f /docker-entrypoint-initdb.d/10_create-psql.sql 
        psql "${PG_DATABASE}" -U postgres -p "${PG_PRIMARY_PORT}" -a -f /docker-entrypoint-initdb.d/20_create-fk-index.sql 
        psql "${PG_DATABASE}" -U postgres -p "${PG_PRIMARY_PORT}" -a -f /docker-entrypoint-initdb.d/30_create-case-insensitive-index.sql 
        cp /docker-entrypoint-initdb.d/99_user_privileges.sql /tmp/99_user_privileges.sql
        sed -i "s/PG_USER/$PG_USER/g" /tmp/99_user_privileges.sql
        psql "${PG_DATABASE}" -U postgres -p "${PG_PRIMARY_PORT}" -a -f /tmp/99_user_privileges.sql 
